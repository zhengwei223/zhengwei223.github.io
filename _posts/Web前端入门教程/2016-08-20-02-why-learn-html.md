...
&emsp;&emsp;“Web”就是“网络”，更准确地说就是“万维网”。

&emsp;&emsp;完整的万维网应用程序分为客户端（前端）和Web服务器程序（后端）。

&emsp;&emsp;人们用Web浏览器（前端）发起请求，获取到存储在某个站点服务器（后端）上的网页（Web Page）文件，由浏览器负责把它读取、展示成外观不一、功能强大的图形界面。

 ![www_mechanism](/public/img/html/www_mechanism.jpg)

&emsp;&emsp;这些成千上万的文档，通过某种形式的连接连接，一起组成了一个巨大的网状结构。

 ![www_web_structure](/public/img/html/www_web_structure.jpg)

&emsp;&emsp;简单地说，学习网页前端的技术，就是在学习怎样能写出能被浏览器顺利读取并且正常展示的网页（Web page）文件。
