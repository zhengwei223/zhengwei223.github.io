...
1. 认识点击事件在网页dom文档的树形结构上的传播过程
2. 通过dom里的event事件对象阻止事件的正常传播
...
事件冒泡在dom上的传播路线：
 ...
 ![js_dom_event_bubbling_app_01](/public/img/js/js_dom_event_bubbling_app_01.gif)
...
### 增加点击事件响应函数
    ...
    <div class="styleClass" onclick="alert('最外层DIV收到点击事件!');>
     DIV Element
     <span class="styleClass"  onclick="alert('SPAN元素收到点击事件');">
     SPAN Element
     <input class="styleClass" type="button" value="我是按钮，快点击我！" 
...
&emsp;&emsp;在一个空白页面的内容区加入以下代码HTML:
...
&emsp;&emsp;此时点击最内层div，三个div的边框颜色会被按照从内到外的顺序逐个修改：

 ![js_dom_event_bubbling_app_03](/public/img/js/js_dom_event_bubbling_app_03.gif)
...
&emsp;&emsp;在浏览器里面测试点击，结果可以看到，由于我们在“event”对象上进行了阻止冒泡有关编码，两个外层div的边框颜色没有发生变化：查看事件冒泡过程被阻断。

 ![js_dom_event_bubbling_app_02](/public/img/js/js_dom_event_bubbling_app_02.gif)
...
&emsp;&emsp;实验四我们主要演示了dom元素的事件如何传播以及如何阻断事件传播。

## dom事件冒泡

### 冒泡路径

&emsp;&emsp;在HTML文档里有为数众多的DOM元素相互嵌套共同组成一个树形结构，当你点击下级DOM子节点的时候，这个节点的上级DOM节点的点击事件也会被触发，这就是所谓的事件冒泡。

&emsp;&emsp;事件冒泡过程的存在已经为实验演示所证实：当最内层按钮被点击之后，一路向上，按钮的上级节点span和div的点击事件被依次触发。

 ![event-order-bubbling](/public/img/js/event-order-bubbling.gif)

&emsp;&emsp;这个点击事件由内向外、自底向上的传播顺序又叫做冒泡书序（bubble order）。

### 函数里的this

&emsp;&emsp;根据实验结果，我们可以推断，每当DOM节点的点击事件处理函数被调用的时候，函数内部的关键字“this”的指向目标就是这个DOM节点本身。

&emsp;&emsp;之前在用对象字面量构建第一个汽车对象的时候，我们通过把对象成员方法内部的“this”与对象变量名进行“===”比较，已得知其成员方法内部的this指的就是汽车对象本身。

&emsp;&emsp;这是一个一般到特殊的思维过程。

&emsp;&emsp;从JavaScript语言编程的角度看，通过*document.getElementById*方法获取的DOM节点本质上只是一个JS对象，我们*通过onclick*属性名添加到对象上面的值也是一个标准的JS函数，一切都很没有意外。

&emsp;&emsp;鼠标点击某个DOM节点时，浏览器会先找到代表页面DOM元素的JS对象，然后调用“onclick”属性指向的函数，在“onclick”属性指向的事件处理函数内部，*this*指向触发事件的JS对象完全合乎逻辑。

&emsp;&emsp;同样的道理，一个直接定义在BOM模型顶层对象*window*下面的函数，它内部的关键字“this”自然要指向*window*对象。

 ![this_key_word_4_dom_el](/public/img/js/this_key_word_4_dom_el.png)
...
&emsp;&emsp;想要干预事件传播过程，就离不开对事件本身进行JavaScript编程。

&emsp;&emsp;我们在实验五里面通过JavaScript代码给多个DIV元素设置“onclick”点击属性。当最内层div元素发生click事件的时候，浏览器会生成一个事件对象，作为参数传给监听函数。

&emsp;&emsp;在阻断事件冒泡的示例代码的第六行加入下面一行代码代码，可以帮助我们查看时间对象的内部结构：

    console.dir(event);

 ![js_dom_event_bubbling_app_04](/public/img/js/js_dom_event_bubbling_app_04.gif)

&emsp;&emsp;根据W3C规范，所有DOM事件对象的类型都会遵循[DOM Event 模型](https://developer.mozilla.org/zh-CN/docs/Web/API/Event),大家可以打开页面链接，跟着下面的步骤找到“MouseEvent”类型的页面：

 ![js_dom_event_bubbling_app_05](/public/img/js/js_dom_event_bubbling_app_05.gif)

&emsp;&emsp;关于阻止事件冒泡的方法，大家可以参考[event.stopPropagation](https://developer.mozilla.org/zh-CN/docs/Web/API/Event/stopPropagation)了解详细内容。
...
&emsp;&emsp;有关使用JavaScript语言操作HTML document的方法，大家可以查看下面的网页链接：

 1.MDN的Web API网页： [JavaScript DOM API](https://developer.mozilla.org/zh-CN/docs/Web/API)

 2.喜欢Java API风格的同学可以查看这个网页地址：[http://krook.org/jsdom/](http://krook.org/jsdom/)
