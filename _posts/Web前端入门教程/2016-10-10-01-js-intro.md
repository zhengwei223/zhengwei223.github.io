...
&emsp;&emsp;日期对象的构造函数是JavaScript标准库函数Date。Date也有一个prototype属性，我们可以通过它给所有日期对象定义新的实例方法。
...
&emsp;&emsp;12小时分段计时法：
    ...
    var h = this.getHours();
      ...
      var result = h.slice(-2) + ":" +m.slice(-2) + ":" +s.slice(-2) + " " + ap
      return result;
    };
...
&emsp;&emsp;实验五我们只做了一件事：修改日期类型对象的构造函数的prototype属性，给对象扩充新方法。

## Date.prototype

根据[MDN JavaScript标准库 Date Date.prototype](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Date/prototype)页面的描述：

>Date 实例继承自Date.prototype。 可以通过修改构造函数的原型对象来影响 Date 实例继承的属性和方法。

### 一切都是对象——Date.prototype

 ![js_extend_date_screen_image_01](/public/img/js/js_extend_date_screen_image_01.gif)

&emsp;&emsp;从控制台的输入信息可以看出：Date.prototype本身也是一个对象。

&emsp;&emsp;此外，我们再来检查一下日期对象类型的“\__proto__”属性的具体值：

 ![js_extend_date_screen_image_02](/public/img/js/js_extend_date_screen_image_02.gif)

&emsp;&emsp;使用等性运算符“===”比较结果为*true*表明：它们其实就是**同一回事**。

## JS语言的继承

### prototype chain

&emsp;&emsp;在JavaScript语言里面，一说到继承，那就是对象的继承。日期对象从构造函数的prototype属性（这是一个对象）继承全部属性和方法。

&emsp;&emsp;以日期类型对象为例，下面的图片，完整地表述了有关的继承关系。

 ![prototype_chain](/public/img/js/prototype_chain.png)

&emsp;&emsp;有关JavaScript语言继承的更深入的讨论和解释，请大家查看[MDN JavaScript 继承与原型链](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Inheritance_and_the_prototype_chain)页面。

## JS Date开源类库

&emsp;&emsp;我们在实验五做的工作，其实早已经有人做过，而且他们还把工作成果发布成了开源的JavaScript日期扩展库。

&emsp;&emsp;大家可以到查看：[JavaScript Datejs日期类库官网](http://www.datejs.com/)查看、下载并且试用。
