...
&emsp;&emsp;当JS引擎在rabbit对象上找不到“eats”属性的时候，它会自动到同一个对象的“\__proto\__”属性上面查找：
 ...
 ![inherit_by_proto](/public/img/js/inherit_by_proto.png){:width="300px" height="300px"}

&emsp;&emsp;我们再来创建一个的对象,用来模拟一个兔子吃东西的过程：

    //它的肚子空了
    var hungaryRabbit = { full : false  };
    hungaryRabbit.__proto__ = rabbit;
    console.log('动物都能进食。一只兔子空着肚子，是不是想吃饭了？'+ rabbit.eats);
    console.log('兔子都会东跳西跳。趁等吃饭的时候，来给同学们跳两下？'+ rabbit.jumps);
    console.dir(hungaryRabbit);
    //兔子吃饱饭之后，记得要修改full的属性值哦
    hungaryRabbit.full =true;
    //同一只兔子，换个名字吧
    var fedUpRabbit = hungaryRabbit;
    //已经吃饱了撑得慌的兔子当然不能继续吃、也不能继续跳了
    fedUpRabbit.eats = false;
    fedUpRabbit.jumps = false;
    console.dir(fedUpRabbit);
    console.log('动物都能进食，一只吃饱了撑的兔子还能不能继续吃？' + fedUpRabbit.eats);
    console.log('兔子都会东跳西跳，一只吃饱撑得的兔子还能不能跳？' + fedUpRabbit.jumps);
