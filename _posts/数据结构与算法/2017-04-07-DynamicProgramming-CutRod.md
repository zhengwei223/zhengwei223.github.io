---
layout: post
title: 高级设计与分析：动态规划法（钢条切割问题）
category: 数据结构与算法
tags: 进阶 数据结构与算法
keywords: 蓝桥 lanqiao 教程 动态规划法
description: 本文将用Java代码实现算法导论所举的钢条最佳切割方案问题。
author: 郑未
---
...
/**
 * 给定一个长度为n英寸的钢条和一个价格表pi(i=1,2,3,...n)表示不同长度的钢条的售价,求能够使销售收益rn最大的切割方案。<br>
 * 
 * 
...
public class CutRod {
	 ...
	 * @return
	 */
	public static int r1(int a[], int n) {
		if (n == 0) {
			return 0;
		}
		int q = -1000;// 保证程序的顺利启动
		for (int i = 1; i <= n; i++) {
			int qq = a[i - 1] + r1(a, n - i);
			if (q < qq) {
				q = qq;
			}
		}
	 ...
	 * @return
	 */
	public static int r_m(int a[], int n) {

		int[] state = new int[n+1];
		state[0] = 0;
		state[1] = a[0];
		return r_m(a, n, state);
	}

	private static int r_m(int a[], int n, int[] state) {
		if (n == 0) {
			return state[0];
		}
		int q = -1000;// 保证程序的顺利启动
		for (int i = 1; i <= n; i++) {
			int x = n-i;
			if (state[x]==0) {
				state[x] = r_m(a, x, state);
			}
			int qq = a[i - 1] + state[x];
			if (q < qq) {
				q = qq;
			}
		}
	...
	public static void main(String[] args) {
		int a[] = { 3, 5, 8, 9, 10, 12, 14, 15, 16, 17, 17, 20, 22, 24, 25, 27, 30, 33, 34, 36, 37, 39, 41, 44, 45, 47, 50,
		    51, 53, 55, 56, 57, 59, 60 };// 钢条分割的价格数组
