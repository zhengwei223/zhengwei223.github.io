...
　　通过前面课程的学习，读者已经掌握了JavaSE、Web前端以及数据库方面的技术。从本书开始，将结合之前所学内容，逐步接触Web系统开发。本章将从动态页面和静态页面的区别说起，介绍Web应用系统的工作原理。读者可以系统地了解并学会使用Tomcat这个非常流行的应用程序服务器，以及使用Eclipse快速的开发Web程序。
...
　　动态网页：页面的内容生成以后，可以随着时间、客户操作的不同而发生改变。比如我们经常使用的百度、淘宝、京东等网站，都是使用的动态网页。以百度为例，当我们搜索“JSP”时，页面会显示与“JSP”相关的内容，如图1.1；而当我们搜索“蓝桥软件学院”，页面则会显示与“蓝桥软件学院”相关的内容，如图1.2。

　　值得注意的是，不要将动态网页和页面内容是否有动感混为一谈。
...
　　C/S 架构：Client/Server，客户端/服务端模式。将软件系统分为客服端和服务端两层，用户在本机安装客户端，通过网络连接服务端。比如微信、QQ、魔兽世界等。现以QQ为例（如图1.3），分析C/S架构的不足：

![](http://i.imgur.com/Gk0wKWK.png)

*图1-03*

1.	如果腾讯将QQ进行了升级，那么全球所有QQ客户端都要进行升级。也就是说，凡是采用了C/S架构的软件，每一次对软件进行微小的改动，所有的客户端都必须升级
2.	任何一台客户端出了问题，都必须进行维护；
3.	所有客户端都必须安装QQ软件。

　　那么能不能别这么麻烦？有没有一种架构，可以即降低我们的维护量，又能更广泛的使用我们的系统呢？答案就是B/S架构。

　　B/S结构：Browser/Server，浏览器/服务器模式。这种模式统一了客户端，将系统功能实现的核心部分集中到服务器上，简化了系统的开发、维护和使用。客户机上只要安装一个浏览器就可访问服务端。

　　比如使用B/S架构实现的网页版百度（如图1.4），用户并不需要安装百度客户端，就能通过浏览器访问百度，类似的还有网页版的淘宝、京东等。即使开发人员对百度进行修改，也不会影响到任何一个用户。也就是说，采用B/S架构的软件系统，用户只要能联网，就可以通过浏览器来访问，系统升级后客户端无需做任何动作。

　　但需要注意，B/S架构是C/S架构的升级或改善，并不是C/S架构的替代品。C/S架构也有很多自己独有的优势，比如C/S架构的软件系统本地响应快、界面更美观友好、能减轻服务器的负荷等。
...
# 1.3Tomcat服务器安装及使用 #
　　在进行Java Web开发时，需要有服务器端的支持，本章就将讲解最常用的Web服务器——Tomcat的安装、配置和工作原理。

　　 软件基金会的Jakarta 项目中的一个核心项目，由Apache、Sun 和其他一些公司及个人共同开发而成。Tomcat 服务器是一个免费的开放源代码的Web 应用服务器，属于轻量级应用服务器，是开发和调试JSP 程序的首选。

## 1.3.1安装tomcat服务器
　　访问Apache官网[http://www.apache.org/](http://www.apache.org/)，点击Tomcat

![](http://i.imgur.com/q1YISVj.png)

*图1-05*

　　在Download下选择Tomcat7.0，

![](http://i.imgur.com/9uGHIs5.png)
...
<table>
   ...
   <tr>
      <td>bin</td>
      <td>存放tomcat服务器的所有可执行命令，包含启动和停止tomcat服务器</td>
   </tr>
   <tr>
      <td>conf</td>
   ...
   <tr>
      <td>work</td> 
      <td>Tomcat服务器把由JSP生成的Servlet文件(*.java)，以及编译产生的字节码文件(*.class)存放在此目录。该文件夹可以删除，但每次启动Tomcat服务器时，系统将重新建立该路径</td>
   </tr>
</table>
...
① Tomcat使用前必须配置JAVA_HOME，详见JAVA书籍。

② Tomcat可以选择性的配置CATALINA_HOME。配置方法：在环境变量里，新建系统变量，变量名设置为CATALINA_HOME,变量值设置为tomcat的安装目录D:\apache-tomcat-8.0.30，如图

![](http://i.imgur.com/vnmb6Wz.png)
...
　　不同的服务器都有自己默认的端口号，Tomcat服务器的默认端口号是8080。某一端口被占用后，该主机上别的应用程序将无法使用此端口。

　　启动Tomcat服务器：双击打开Tomcat安装目录里，bin目录中的启动命令startup.bat，如图，

![](http://i.imgur.com/5v1rKz9.png)

*图1-10*

　　然后打开浏览器输入[http://localhost:8080/](http://localhost:8080/)，即可访问Tomcat服务器的主页面，如图，
...
③将其中的默认端口8080，改为其他未被占用的端口，如8888。

　　前面讲过，tomcat安装文件夹中的webapps目录，里面保存着可以运行的Web项目。Tomcat下载后，webapps里面就自带了几个可以直接运行的示例项目，如图

![](http://i.imgur.com/wg20Eh6.png)

*图1-12*

　　我们先执行tomcat的启动命令startup.bat，然后在浏览器输入[http://localhost:8888/examples/](http://localhost:8888/examples/)，即可访问webapps中的examples项目，如图

![](http://i.imgur.com/lh8KsTr.png)

*图1-13*

　　我们实际访问的是examples文件夹中的index.html文件。如果将index.html文件删除，再次访问[http://localhost:8888/examples](http://localhost:8888/examples/)，页面就会提示错误信息，如图
...
　　将其中的false改为true即可。改完之后，重新启动tomcat服务器（先执行shutdown.bat关闭，再执行startup.bat启动），再次访问[http://localhost:8888/examples/](http://localhost:8888/examples/)，则就可以得到examples中的文件目录结构，如图
...
　　前面我们已经创建好了一个名为JspProject的项目结构，并且指定了该项目的默认首页是index.jsp。接下来，我们在JspProject根目录里创建一个index.jsp文件，内容如下， \apache-tomcat-7.0.37\webapps\JspProject\index.jsp
...
　　然后运行startup.bat启动tomcat服务器，并在浏览器输入[http://localhost:8888/JspProject/](http://localhost:8888/JspProject/)，就可以成功运行此项目，如图
...
## 1.3.5.使用Eclipse开发Web项目

　　我们还可以借助Eclipse等开发工具来快速、高效地开发Web项目。Eclipse是一个免费的IDE（集成开发环境）工具，是目前最受欢迎的Java EE开发工具。

　　可以登录Eclipse官网[http://www.eclipse.org/](http://www.eclipse.org/)，并单击Download按钮，如图

![](http://i.imgur.com/3lN7tos.png)

*图1-22*

　　然后根据自己的操作系统，选择32位或64位的版本进行下载(本书采用64位)，如图

![](http://i.imgur.com/8M9ySth.png)

*图1-23*

　　最后再按照提示下载(例如，可选择MD5→DOWNLOAD)，如图

![](http://i.imgur.com/3szHTZh.png)

*图1-24*

　　下载完后，解压，然后直接运行里面的eclipse.exe即可（运行时，会提示让选择项目的存放路径）。

　　第一次进入eclipse，会显示欢迎页面，直接关闭即可，如图

![](http://i.imgur.com/Mdtb1tI.png)

*图1-25*

　　为了能用Eclipse开发Web项目，我们还需要给Eclipse配置Tomcat服务器，具体步骤如下：

1.依次点击Windows→Preferences→Server→Runtime Environment→Add…→Apache Tomcat v8.0→next，如图

![](http://i.imgur.com/qGJd8dQ.png)

*图1-26*

2.选择Tomcat的安装目录：依次点击Browse…→选择tomcat8的安装目录→　确定，如图，

![](http://i.imgur.com/OnWlVkJ.png)

*图1-27*

3.选择tomcat所依赖的JRE，如图，

![](http://i.imgur.com/bUIMwNq.png)

*图1-28*

　　JRE可以从下拉列表选择JDK，或者点击Installed JREs..进行配置。

4.设置JSP文件编码。为了防止乱码的产生，我们可以将JSP文件统一设置为UTF-8格式：依次点击Windows→　Preferences→Web→JSP Files→将Encoding选为UTF-8，如图，

![](http://i.imgur.com/8R7ITwG.png)

*图1-29*

　　之后，就可以开始用Eclipse来创建Web项目了，具体步骤如下：

1.依次点击File→　New→　Dynamic Web Project，如图，

![](http://i.imgur.com/UgticFe.png)

*图1-30*

2.在Project name后面输入项目名（如JspProject），再将Target runtime选为tomcat7，如图，

![](http://i.imgur.com/C7FPkZJ.png)
...
3.点击Servers控制台，进行配置指定Tomcat，如图,

![](http://i.imgur.com/u3vyOEn.png)

*图1-32*

　　-------------------------》部署

4.将之前创建的JspProject项目加入到Tomcat中：鼠标右键点击Servers中的Tomcat→ Add and Remove…→ 选择JspProject→ Add→ Finish，如图，

![](http://i.imgur.com/9qYA2F5.png)

*图1-33*

5.创建Jsp文件：鼠标右键单击WebContent->　New→　Jsp File→　输入jsp名字index.jsp→　Finish

6.编写Jsp文件，输出Hello Wolrd，如下，
index.jsp
...
7.启动Tomcat服务：右键Servers中的Tomcat→ start

![](http://i.imgur.com/B9hE6mk.png)

*图1-34*

8.访问[http://localhost:8080/JspProject/](http://localhost:8080/JspProject/)，如图,

![](http://i.imgur.com/Mwma3vI.png)

*图1-35*

9.若要关闭Tomcat服务，则直接点击控制台的关闭按钮，如图,

![](http://i.imgur.com/f6j3mZM.png)
...
2.使用Eclipse开发一个Web项目，通过项目中的JSP文件输出“Hello World”。
...
BS：浏览器/服务器，比如京东网站。京东网站进行的各种维护、升级，客户端不需要升级。
 
CS：客户端/浏览器，比如QQ的pc版。QQ的每一次升级，都会涉及全部QQ用户的软件更新。
...
在Eclipse中:window-->preferences -->Server

-->Runtime Environments-->add-->Apache Tomcatv7.0
...
设置项目的运行时环境：项目右键-Properties-Targeted Runtimes--勾选运行环境
至此项目建立完毕。
...
部署项目：在Servers标签中，点击“No Servers...”-->选中tomcat7-->next
