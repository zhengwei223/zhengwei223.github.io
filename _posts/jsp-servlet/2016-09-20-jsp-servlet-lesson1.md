...
3.	使用Tomcat开发一个Web项目时，Web项目默认存放在Tomcat的哪个目录下，能否更换此目录？Web项目的目录结构有什么要求？
4.	如何修改Tomcat的端口号？
5.	使用Tomcat手动部署一个Web项目，通过项目中的JSP文件输出“Hello World”。
6.	如何在Eclipse中集成Tomcat？
7.	使用Eclipse集成Tomcat部署一个Web项目，通过项目中的JSP文件输出“Hello World”。
8.	能否通过www.helloword.com的形式访问本地项目？
9.	如何给Tomcat添加管理员？
10.	简述JSP的执行流程。
11.	如何通过Eclipse设置JSP文件的默认编码及文本内容编码？
